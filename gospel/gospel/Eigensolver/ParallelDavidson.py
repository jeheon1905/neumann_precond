...
def davidson(
    ...
    with Timer.track("Rotation", timing, verbosity):
        if use_MP:
            _dtype = SP if MP_scheme in [1, 4, 5] else FP
            C = C.to(_dtype)
        else:
            _dtype = None
        X_ = X_.to(_dtype) @ C
    ...
    for i_iter in range(2, maxiter + 1):
        ...
        # print(f'before empty_cache memory usage: {torch.cuda.memory_allocated(device)}')
        torch.cuda.empty_cache()  # TODO: Check its effect
        # print(f'after empty_cache memory usage: {torch.cuda.memory_allocated(device)}')
        ...
        while True:
            ...
            with Timer.track("Orthonormalize R", timing, verbosity):
                if use_MP:
                    if MP_scheme in [1, 2, 3]:
                        _dtype = SP
                    elif MP_scheme in [4, 5]:
                        _dtype = FP
                        R_ = R_.to(_dtype)
                    else:
                        _dtype = None
            ...
            with Timer.track("Projection (R.H @ AU)", timing, verbosity):
                if use_MP:
                    _dtype = SP if MP_scheme in [1, 2, 3] else FP
                else:
                    _dtype = None
