#!/bin/bash -l
#SBATCH --job-name=ab_0.9_N10
#SBATCH --output=/home/jhwoo1905/Projects/neumann_precond/results_paper/alpha_ablation/jobs/B12_a0.9_N10_%j.txt
#SBATCH --partition=normal
#SBATCH --nodes=1 --ntasks-per-node=1 --cpus-per-task=8 --time=12:00:00 --exclusive
# One (alpha, order) per job.  The single-mode factor predicts over-correction only (mu = +3.92), i.e. a normal run here
...
echo "host: $(hostname)  commit: $(git rev-parse HEAD)  system: B12  alpha: 0.9  order: 10"
echo "########## B12 alpha=0.9 order=10 ##########"
T0=$SECONDS
...
echo "########## B12 alpha=0.9 order=10 DONE $((SECONDS-T0)) s ##########"
